#!/usr/bin/env bash
#
# setup_ffmpeg.sh - FFmpeg Installation Script
# Description: Installs FFmpeg, a complete, cross-platform solution to record, convert and stream audio and video
# Category: Utilities
# Usage: ./setup_ffmpeg.sh [OPTIONS]
#        -y, --yes, --non-interactive    Skip confirmation prompts
#        -h, --help                      Show help message
#        Can also be run remotely with: bash <(curl -fsSL https://raw.githubusercontent.com/user/repo/main/installers/setup_ffmpeg.sh)
...
source_library() {
    ...
    if is_running_remotely; then
        ...
        local script_url
        script_url=$(curl -fsSL -w "%{url_effective}\n" -o /dev/null "https://raw.githubusercontent.com/$repo_user/$repo_name/$repo_branch/installers/setup_ffmpeg.sh" 2>/dev/null || echo "")
        ...
        if [[ -n "$script_url" ]] && [[ "$script_url" == *"raw.githubusercontent.com"* ]]; then
            ...
            if [[ -n "$url_branch" ]] && [[ "$url_branch" != "setup_ffmpeg.sh" ]]; then
...
main() {
    ...
    if command -v ffmpeg &> /dev/null; then
        ...
        log_warning "FFmpeg is already installed (version: $installed_version)"
    ...
    # Install FFmpeg using the dependency management functions
    if install_package "ffmpeg"; then
        log_success "===================================="
        log_success "FFmpeg installation completed!"
        log_success "===================================="
        echo
        echo "FFmpeg is now ready to use. Try it out:"
        echo "  ffmpeg -version"
        echo "  ffmpeg -h"
        echo
        echo "Basic usage examples:"
        echo "  ffmpeg -i input.mp4 output.avi"
        echo "  ffmpeg -i input.mp4 -vf scale=1280:720 output.mp4"
        echo
        exit 0
    else
