...
#        Can also be run remotely with: bash <(curl -fsSL https://raw.githubusercontent.com/user/repo/main/installers/setup_fabric.sh)
...
source_library() {
    ...
    if is_running_remotely; then
        ...
        local script_url
        script_url=$(curl -fsSL -w "%{url_effective}\\n" -o /dev/null "https://raw.githubusercontent.com/$repo_user/$repo_name/$repo_branch/installers/setup_fabric.sh" 2>/dev/null || echo "")
        ...
        if [[ -n "$script_url" ]] && [[ "$script_url" == *"raw.githubusercontent.com"* ]]; then
            ...
            if [[ -n "$url_branch" ]] && [[ "$url_branch" != "setup_fabric.sh" ]]; then
